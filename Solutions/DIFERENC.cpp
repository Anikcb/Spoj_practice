...
void build(ll N)
{
    for (int i=0; i<N; i++)STmn[0][i] = i,STmx[0][i] = i;
    for (int k = 1; (1 << k)<N; k++)
    {
        for (ll i=0; i+(1<<k)<=N; i++)
        {
            int x1 = STmn[k-1][i];
            int y1 = STmn[k-1][i+(1<<k-1)];
            int x2 = STmx[k-1][i];
            int y2 = STmx[k-1][i+(1<<k-1)];
            STmn[k][i]=A[x1]<=A[y1]?x1:y1;
            STmx[k][i]=A[x2]>=A[y2]?x2:y2;
        }
    }
}

ll Mnquery(int i, int j)
{
    if(i==j)return i;
    int k = log2(j-i);
    int x = STmn[k][i];
