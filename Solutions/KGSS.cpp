...
{
    ll sum=0;
    ll p=query(1,1,n,a,b);
    if(p==a || p==b){
        if(p==a)sum+=arr[query(1,1,n,a+1,b)];
        else sum+=arr[query(1,1,n,a,b-1)];
    }
    else sum+=max(arr[query(1,1,n,a,p-1)],arr[query(1,1,n,p+1,b)]);
    return sum+arr[p];
}
